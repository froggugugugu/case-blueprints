...
Q1. 屋外で使う?
  YES → Q2. 高温(車内・夏季屋外)?
    YES → ABS / ASA(後者は耐 UV を含む、ASA は data/materials/ 未収録)
    NO  → PETG(UV 安定剤入りグレード推奨)
  NO  → Q3. 落下/反復動作?
    YES → PLA+
...
直射日光・振動・雨天・温度差を全て受ける用途は、PLA / PLA+ は **不可**。
材料別の現実的な評価:

| material | 屋外可 | 真夏車内 | 振動下のねじ | UV 耐性 | コメント |
|---|---|---|---|---|---|
| PLA / PLA+ | ❌ | ❌ | ⚠ heat-set 必須 | ❌ | 60℃ 軟化、紫外線で脆化 |
| PETG | ✅ | △ | ⚠ heat-set 推奨 | ⚠ UV 添加剤次第 | 一般的な選択肢、衝撃強度 ◯ |
| ABS | ✅ | ✅ | ✅ heat-set 良好 | ⚠ ASA 推奨 | アセトン蒸気研磨で表面密に |
| TPU | サブ専用 | サブ専用 | — | — | パッキン・防振マウントに |

P19(振動でねじ緩む)は **heat-set 真鍮インサート + ロック剤**で対処。
P20(熱変形)は **明色** + **ventilation 追加**で対処。

詳細は `@.claude/pitfalls.md` の P18(防水)/ P19(振動)/ P20(熱変形)を参照。
